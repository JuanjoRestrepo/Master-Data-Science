...
\section{Preprocesamiento y Control de Calidad de Datos}
...
\begin{itemize}
    ...
    \item \textbf{Mecanismo de Atención:} Se implementó un bloque de atención tipo \textit{dot-product} modificado. Este mecanismo genera un peso de importancia $\alpha_i$ para cada parche $i$ a partir de su embedding $h_i$. Matemáticamente:
        \begin{equation*}
            a_i = w^{\top} \tanh(V h_i^{\top}), \qquad \alpha_i = \frac{\exp(a_i)}{\sum_j \exp(a_j)}
        \end{equation*}
    La representación global de la bolsa (WSI) se obtiene mediante la suma ponderada:
        \begin{equation*}
            H = \sum_i \alpha_i h_i
        \end{equation*}
